...
def main():
    ...
    # checkpoint (suffix distinguishes 2-NODE variant from 1-NODE)
    _ckpt_suffix = '_N2' if getattr(args, 'second_node', False) else ''
    _ckpt_name = (
        args.model_type
        + '-'
        + str(args.dataset)
        ...
        + str(args.fold)
        + '_D'
        + str(args.drop if args.drop is not None else '0')
        + '_S'
        + str(args.random_seed)
        + _ckpt_suffix
        + '.ckpt'
    )
    ckpt_path = os.path.join(args.save, _ckpt_name)
    # set logger
