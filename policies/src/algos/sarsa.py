...
from src.utils.utils import set_seeds, make_env, get_mask, encode_state, ReplayBuffer, TransitionSARSA, get_episode_stats
from src.utils.models import QNetwork, linear_schedule



def run_sarsa(args, use_tensorboard=False, use_wandb=False, return_agent=False):
    ...
    run_name = f"sarsa_{args.seed}_{int(time.time())}"
    if use_wandb:
        import wandb 
        wandb.init(
    ...
    # sample action
    epsilon = linear_schedule(args.start_e, args.end_e, args.exploration_fraction * args.max_episodes, 0)
    ...
    while episode_number < args.max_episodes:
        ...
        if random.random() < epsilon:
            # actions = np.array([envs.single_action_space.sample() for _ in range(envs.num_envs)])
            # pick a random action from allowed_action list
            next_actions = np.array([random.choice(allowed_actions) for _ in range(envs.num_envs)])
        else:
            q_values = q_network(obs.to(device), action_masks.to(device))
            next_actions = torch.argmax(q_values, dim=1).cpu().numpy()

        # TRY NOT TO MODIFY: record rewards for plotting purposes
        if terminated:
            # print(start_index, global_step)
            total_reward, episode_length = get_episode_stats(infos)
        ...
        next_actions = torch.Tensor(next_actions).reshape((1,-1)).long()
        ...
        data = TransitionSARSA(obs, actions, action_masks, rewards, next_obs, next_actions, next_action_masks, terminated)
        ...
        with torch.no_grad():
            target_q = q_network(data.next_state).gather(1, data.next_action).view(-1)
            td_target = (data.reward.flatten() + args.gamma * target_q * (1 - data.done.flatten())).view(-1)
        old_val = q_network(data.state).gather(1, data.action).view(-1)
...
if __name__ == '__main__':
    ...
    run_sarsa(args, use_tensorboard=True, use_wandb=args.track)
