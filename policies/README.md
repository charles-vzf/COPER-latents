...
```
## Integration in `code/COPER`

This directory is now kept as an in-project copy of the upstream baseline
repository so policy training, evaluation, and plotting can happen directly
inside `code/COPER`.

Notes:

- the local `requirements.txt` from the original repo has been removed on
  purpose
- dependencies needed by this copied code should be declared in the global
  `code/COPER/requirements.txt`
- the main notebook entry point for quick experimentation and analysis is
  `code/COPER/notebooks/train_mdp_policies.ipynb`
