...
def init_args():
    ...
    parser.add_argument(
        '--second-node',
        action='store_true',
        help='Apply a second Neural ODE after the Perceiver on the latent grid (2-NODE variant). '
        'Default (flag off) is 1-NODE: ODE only before the Perceiver.',
    )
    ...
    if args.save == 'results/checkpoints/':
        args.save = os.path.join(args.results_dir, 'checkpoints')
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint.pt', trace_func=print, logger=None):
        ...
        # NumPy 2.0 removed `np.Inf` in favor of `np.inf`.
