...
Implementation largely follows the original COPER reference repository.
...
| `COPERvsTRANSFORMER_mortality.ipynb` | Train and compare **COPER vs Transformer** on MIMIC mortality tensors (default **no input-time** `--drop`; `NITERS_LIST` typically **1, 3, 10** — keep in sync with `display_embeddings.ipynb`). Also fits **logistic (L2/L1), random forest, PyTorch LSTM**; saves `.joblib` / `.pt` baselines under `results/demo_outputs/coper_vs_transformer_mortality/models/`. Exports deep bundles to `results/runs/<date>_mimic_compare/models/` (optional pointer file `results/latest_mimic_compare_models_dir.txt`). Tables → `.../tables/`. |
| `mortality_maps_mdp_states.ipynb`    | **State-wise mortality maps:** cohort (patient- and row-level) vs tabular MDP (optimal policy, closed-form + Monte Carlo) vs exported deep bundles + classical baselines, on **mean pickle tensor per MDP state**; similarity table + correlation heatmap. Requires unified `mdp_params_<slug>/`, mortality pickle, and prior `COPERvsTRANSFORMER_mortality.ipynb` exports.                                                                                                                                                                                                 |
...
| `coper_to_states.ipynb`              | Single-slot latent **z** → histogram over MDP cluster states; head → `results/demo_outputs/coper_to_states/models/`.                                                                                                                                                                                                                                                                                                                                                        |
| `copers_to_states.ipynb`             | Full latent grid `(L,D)` vs MDP state per bloc (frozen COPER); shared linear head + trajectory plots → `results/demo_outputs/copers_to_states/`.                                                                                                                                                                                                                                                                                                                           |
| `train_coper_to_mdp.ipynb`           | Train **COPER end-to-end** on per-timestep MDP labels (CE); optional warm-start from mimic-compare exports; optional `USE_POS_ENCODING` (see `src_coper/coper_model.py`). Outputs → `results/demo_outputs/train_coper_to_mdp/`.                                                                                                                                                                                                                                              |
